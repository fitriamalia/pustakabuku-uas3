...
export default {
  ...
  methods: {
    getDipinjam() {
      const url = selectbystatusdipinjam + '?status_peminjaman=' + this.$route.params.status_peminjaman;
      axios
          .get(url)
          .then((resp) => {
            console.log(resp);
            this.readalldipinjam = resp.data;
          })
          .catch((err) => {
            console.log(err);
          });
...
<template>
  ...
  <div class="intro">
    <div>
      <div class="welcome">Tampilan Status Buku Dipinjam</div>
    </div>
    ...
    <div class="container d-flex flex-column align-items-lg-center">
      <table>
        ...
        <tbody>
        <tr v-for="data in readalldipinjam" >
